...
namespace Assets.Scripts.EventBus
{
    public static class EventBus<T> where T : IEvent
    {
        ...
        public static void UnregisterByOwner(object owner)
        {
            ...
            List<IEventBinding<T>> bindingsToRemove = new List<IEventBinding<T>>();
            foreach (IEventBinding<T> binding in SBindings.Keys)
            {
            ...
            foreach (IEventBinding<T> binding in bindingsToRemove)
            {
                SBindings.TryRemove(binding, out _);
            }
        }
