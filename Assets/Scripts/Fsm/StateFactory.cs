...
namespace Assets.Scripts.Fsm
{
    public class StateFactory<TContext> where TContext : class
    {
        ...
        public TState GetState<TState>() where TState : class, IState<TContext>, new()
        {
            ...
            if (!_states.TryGetValue(type, out IState<TContext> state))
            {
                try
                {
                    ...
                    // Debug.Log($"[StateFactory] Created and cached state: {type.Name}");
                }
                catch (Exception ex)
                {
                    Debug.LogError($"[StateFactory] Failed to create state of type {type.Name}. Ensure it has a parameterless constructor and Setup method works correctly. Error: {ex}");
        ...
        public TState GetState<TState>(Func<TState> factoryMethod) where TState : class, IState<TContext>
        {
            ...
            if (!_states.TryGetValue(type, out IState<TContext> state))
            {
                try
                {
                    ...
                    // Debug.Log($"[StateFactory] Created (via factory method) and cached state: {type.Name}");
                }
                catch (Exception ex)
                {
        ...
        public void ClearCache()
        {
            ...
            // Debug.Log("[StateFactory] State cache cleared.");
        }
    }
}